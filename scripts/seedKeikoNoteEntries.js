// お稽古ノート（茶道教室）の初期データ（シード）をFirestoreに投入するスクリプト。
// すでに存在するid（seed-...）は上書きしない。新規追加のみ行う。
// 使い方： node scripts/seedKeikoNoteEntries.js         … 投入内容の確認のみ（dry-run）
//         node scripts/seedKeikoNoteEntries.js --apply … 実際に書き込む
...
async function main() {
  ...
  const col = db.collection("keikoNoteEntries");
  const existing = await col.get();
  const existingIds = new Set(existing.docs.map((d) => d.id));

  const toAdd = KEIKO_NOTE_SEEDS.filter((e) => !existingIds.has(e.id));

  console.log(`既存のお稽古ノート件数: ${existingIds.size}`);
  console.log(`追加対象（未登録のシード）: ${toAdd.length}件`);
  toAdd.forEach((e) => console.log(`  - ${e.id}（${e.date}）`));
